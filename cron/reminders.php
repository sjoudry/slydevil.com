<?php
// @todo Need to retofit the code.
// @todo Need to use mailgun API.
exit;

include_once(realpath(__DIR__ . "/../includes") . "/init.inc.php");
...
$date_start     = mktime(0, 0, 0, 12, 1, date("Y"));
$gst            = 0.05;
$pst            = 0.10;


// Load Packages
$result = Query("SELECT * FROM package")->result();
...
// Load Accounts
//$result = Query("SELECT * FROM account JOIN user USING (account_id) ORDER BY account_date_added, account_name")->result();
$result = Query("SELECT * FROM account JOIN user USING (account_id) ORDER BY account_name DESC")->result();
while ($row = $result->fetch_assoc())
{
...
// Load Domains
$result = Query("SELECT * FROM domain ORDER BY domain_date_added, domain_name")->result();
while ($row = $result->fetch_assoc())
{
...
// Load Services
$result = Query("SELECT * FROM service")->result();
while ($row = $result->fetch_assoc())
{
...
// Load Invoices
$result = Query("SELECT * FROM invoice")->result();
while ($row = $result->fetch_assoc())
{
...
$mail->SMTPAuth   = TRUE;
$mail->Port       = 587;
$mail->Username   = "postmaster@billing.slydevil.com";
$mail->Password   = "6507p68ll276";
$mail->SetFrom("invoice@billing.slydevil.com", "Sly Devil Web Hosting");
$mail->AddBCC("sj@slydevil.com");
...
foreach ($accounts as $account_id => $account)
{
    ...
    if ($create_invoice)
    {
        ...
        {
            $address .= "<br />" . $account["account_postal"];
        }
...
EOS;
        ...
        Query(
            ...
            array(
                Login::crypt_password($invoice_number . time()),
                $invoice_number,
                $account_id,
        ...
        foreach ($invoice_data as $index => $row)
        {
            ...
            $subtotal = round($row["quantity"] * $row["fee"], 2);
            $total   += $subtotal;
...
EOS;
        ...
        $mail->Subject    = "New Web Hosting Invoice - " . sprintf("SDWH-%05d", $invoice_number);
        $mail->AddAddress($invoice_number . "@slydevil.com", "Scott Joudry");
