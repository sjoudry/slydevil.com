...
class SessionManager {
	...
	public function checkFormToken(string $form_id, string $form_token) {
		$form_token = $this->filterVariable($form_token);

		if (isset($_SESSION[self::SESSION_FORM_TOKEN_NAME][$form_id][$form_token])) {
			if ((time() - $_SESSION[self::SESSION_FORM_TOKEN_NAME][$form_id][$form_token]) > self::SESSION_FORM_TOKEN_MAX_AGE) {
				unset($_SESSION[self::SESSION_FORM_TOKEN_NAME][$form_id][$form_token]);
			}
			else {
				return TRUE;
			}
